...
SELECT * FROM worker WHERE name_ != 'ДЖОКЕР' ORDER BY shop_id ASC, position ASC;

SELECT shop_id, product_id, sum(quanity) as 'КОЛИЧЕСТВО ПИВА' from warehouse where prodoct_id < 4 GROUP BY shop_id ORDER BY 'КОЛИЧЕСТВО ПИВА' DESC;
